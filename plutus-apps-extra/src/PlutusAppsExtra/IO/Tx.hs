...
import qualified PlutusAppsExtra.IO.Tx.Blockfrost    as Blockfrost
...
data TxProvider = Cardano | Maestro | Blockfrost
...
class (HasWalletProvider m, HasChainIndexProvider m) => HasTxProvider m where
    ...
    signTx ctx = getTxProvider >>= \case
        ...
        Blockfrost -> signTxWithoutNode ctx
    ...
    balanceTx params lookups cons mbMetadata = getTxProvider >>= \case
        ...
        Blockfrost -> balanceTxWithoutNode params lookups cons mbMetadata

    submitTx :: CardanoTx -> m ()
    default submitTx :: (MonadBlockfrost m, MonadMaestro m) => CardanoTx -> m ()
    submitTx ctx = getTxProvider >>= \case
       Cardano    -> Cardano.submitTx ctx
       Maestro    -> void $ Maestro.turboSubmitTx ctx
       Blockfrost -> void $ Blockfrost.submitTx ctx
