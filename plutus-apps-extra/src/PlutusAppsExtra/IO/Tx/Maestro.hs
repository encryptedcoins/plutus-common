    ...
    where
        go = do
            getTxState txId <&> fmap tsrState >>= \case
                Nothing -> liftIO (putStrLn "nothing") >>  go
                Just Onchain -> liftIO (putStrLn "Onchain") >>  pure ()
                Just Pending ->  liftIO (putStrLn "Pending") >> liftIO (threadDelay 1_000_000) >> go
                Just state   ->  liftIO (print state) >>throwM (FailedToSubmit state)